      ...
      >
        {firstFive.map(({ id }) => (
          <AccordionItem
            key={id}
            value={`question-${id}`}
            className="border-none rounded-[50px] bg-[rgba(18,18,18,0.26)] shadow-[inset_0_4px_13px_0_rgba(255,255,255,0.25)]"
          >
        ...
        {isExpanded &&
          remaining.map(({ id }) => (
            <AccordionItem
              key={id}
              value={`question-${id}`}
              className="border-none rounded-[50px] backdrop-blur-sm bg-[rgba(18,18,18,0.26)] shadow-[inset_0_4px_13px_0_rgba(255,255,255,0.25)]"
            >
              <AccordionTrigger className="h-[96px] cursor-pointer items-center group px-[32px] py-[31px] text-[14px] md:text-[20px] text-white font-montserrat font-light leading-[120%]">
                <span className="pr-4 text-left leading-snug">
                  {t(`q${id}.question`)}
